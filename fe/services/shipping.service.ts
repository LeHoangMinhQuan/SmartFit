...
export const shippingService = {
  ...
  getProvinces: () =>
    api.get<Province[]>("/api/shipping/provinces").then((r) => r.data),

  getDistricts: (province_id: number) =>
    api
      .get<District[]>(`/api/shipping/districts/${province_id}`)
      .then((r) => r.data),

  getWards: (district_id: number) =>
    api.get<Ward[]>(`/api/shipping/wards/${district_id}`).then((r) => r.data),

  // Available GHN service tiers for the given route
  getServices: (body: ServicesBody) =>
    api
      .post<ShippingService[]>("/api/shipping/services", body)
      .then((r) => r.data),

  // Estimated fee for a specific service + parcel dimensions
  estimateFee: (body: FeeBody) =>
    api.post<FeeEstimate>("/api/shipping/fee", body).then((r) => r.data),

  // Latest row from shipping_logs for a given tracking code
  trackOrder: (tracking_code: string) =>
    api
      .get<ShippingLog[]>(`/api/shipping/track/${tracking_code}`)
      .then((r) => r.data),
};
