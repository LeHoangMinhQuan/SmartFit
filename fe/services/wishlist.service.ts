...
export const wishlistService = {
  ...
  getWishlist: () =>
    api.get<WishlistItem[]>("/api/users/me/wishlist").then((r) => r.data),
  ...
  addToWishlist: (body: { product_id: number; variant_id: number }) =>
    api.post<WishlistItem>("/api/users/me/wishlist", body).then((r) => r.data),
  ...
  removeFromWishlist: (product_id: number, variant_id: number) =>
    api
      .delete(`/api/users/me/wishlist/${product_id}/${variant_id}`)
      .then((r) => r.data),
};
